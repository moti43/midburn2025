...
#define THRESHOLD 1500       // Minimum value to consider it a "hit"
...
void runningLights(int x)
{
  ...
  for (int j=0;j<49;j++)
  {
    if (leds_B[j+1][0]+leds_B[j+1][1]+leds_B[j+1][2]>0){
      leds_B[j+1][0]=0;
      leds_B[j+1][1]=0;
      leds_B[j+1][2]=0;
      color=HeatColor(j*5);
...
void lowLight(int hight,int step)
{
  CRGB color;
  if (step<4)
  {
    for (int j=0;j<49;j++)
    {
      color=ColorFromPalette(currentPalette, hight*10+j, hight*10+j, currentBlending);
      if (j<(hight-(3-step)/3*hight)) leds_B[j]=color;
      else leds_B[j]=0;
  ...
  else
  {
    if (step<hight)
    {
      ...
      {
        color=ColorFromPalette(currentPalette, hight*10+j, hight*10+j, currentBlending);
        if (j<(hight-(step-3)/6*hight)) leds_B[j]=color;
        else leds_B[j]=0;
...
/*
void song1(int starting){
  static int time=0;
  int current_time=0;
  static int runningLowfire=0; static int runningLowlight=0; static int runningBeating=0; static int runningBeatingRed=0;

  if (starting==1)
  {
    time = millis;
  }
  current_time=millis-time;
...
// Begining 3 00:25 - 
  if (current_time>50000 && current_time<120000)
  {
    ...
    currentPalette = HeatColors_p; // RainbowColors_p, OceanColors_p, ForestColors_p, CloudColors_p, HeatColors_p, PartyColors_p.

    EVERY_N_MILLISECONDS(200){runningLowfire=1;ind3=0;Fire2012(1);}
    EVERY_N_MILLISECONDS(20){
      static int ind1=0;
      if (runningLowfire==1){
        Fire2012(0);
        ind1++;
        if (ind1>120) runningLowfire=0; 
      }
    }    
  }

// Begining 2 00:25 - 
  if (current_time>25000 && current_time<120000)
  {
    static int ind2=0;
    if (current_time<100000) currentPalette = CloudColors_p; // RainbowColors_p, OceanColors_p, ForestColors_p, CloudColors_p, HeatColors_p, PartyColors_p.
    EVERY_N_MILLISECONDS(200){runningLowlight=1;ind2=0;}
    EVERY_N_MILLISECONDS(20){
      if (runningLowlight==1){
        lowLight(15,ind2);
        ind2++;
        if (ind2>10) runningLowlight=0; 
      }
    }    
  }

  // Begining 1 00:00 - 00:25
  if (current_time>100 && current_time<100000)
  {
    currentPalette = OceanColors_p; // RainbowColors_p, OceanColors_p, ForestColors_p, CloudColors_p, HeatColors_p, PartyColors_p.
    EVERY_N_MILLISECONDS(20000){runningBeating=1;shortBeating(1);}
    EVERY_N_MILLISECONDS(200){
      static int ind1=0;
      if (runningBeating==1){
        shortBeating(0);
        ind1++;
        if (ind1>10) runningBeating=0; 
      }
    }
  }  

  // Begining 4 00:00 - 00:25
  if (current_time>100000 && current_time<120000)
  {
    currentPalette = HeatColors_p; // RainbowColors_p, OceanColors_p, ForestColors_p, CloudColors_p, HeatColors_p, PartyColors_p.
    EVERY_N_MILLISECONDS(20000){runningBeatingRed=1;Beating(1);}
    EVERY_N_MILLISECONDS(200){
      static int ind4=0;
      if (runningBeatingRed==1){
        Beating(0);
        ind4++;
        if (ind1>60) runningBeatingRed=0; 
      }
    }
  }  
